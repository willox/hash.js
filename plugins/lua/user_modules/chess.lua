...
local function getteam(sid64)
	return (teams[BLACK] == sid64 and BLACK) or (teams[WHITE] == sid64 and WHITE) or false
end

local function movepiece(sid64, oldxy, xy)
	if not (teams[BLACK] and teams[WHITE]) then err("Game not started.") return end
	if not getteam(sid64) then err("You are not in this game.") return end

	if turn ~= getteam(sid64) then err("It's not your turn!") return end
	...
	local curspot = board[oldy][oldx]
	if (getteam(sid64) == BLACK and curspot > 0) or (getteam(sid64) == WHITE and curspot < 0) then err("You may not move the enemy's piece!") return end
...
hook.Add("Message", "CHESSAGE", function(ply, sid64, msg)
	...
	if subcmd == "join" then
		...
		if teams[BLACK] and teams[WHITE] then chessprint("Game ready.") end
	elseif subcmd == "leave" then
		if not getteam(sid64) then err("You are not playing.") return end
	...
	elseif subcmd == "help" then
		chessprint("Commands: join, leave, print, status, reset, help", "To move: !chess <old XY> <XY>")
	else
		if not getteam(sid64) then err("You are not playing.") return end
